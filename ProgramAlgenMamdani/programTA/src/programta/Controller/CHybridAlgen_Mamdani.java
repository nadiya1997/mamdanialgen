...
public class CHybridAlgen_Mamdani {
    ...
    double[][] f_anggota_output, f_anggota_populasi_terurut_cr;
    double[][] fitness_index;
    double[][] final_fitness_index;
...
//          +++++++++ Mengecek Kondisi Cuaca Berdasarkan Hasil Deffuzifikasi ++++++++++++++      
    ...
    public void do_crossover(double cr) {
        ...
        for (int i = 0; i < fitness_index.length; i++) {
            for (int j = 0; j < 2; j++) {
                System.out.print(fitness_index[i][j] + " ");;
            }
            System.out.println();
        }
        ...
        f_anggota_populasi_terurut_cr = new double[f_anggota_populasi_terurut.length][f_anggota_populasi_terurut[0].length];
        f_anggota_populai_temp = new double[idx_k_list.size()][f_anggota_populasi_terurut[0].length - posisi_cross];

        for (int i = 0; i < f_anggota_populasi_terurut_cr.length; i++) {
            for (int j = 0; j < f_anggota_populasi_terurut_cr[i].length; j++) {
                f_anggota_populasi_terurut_cr[i][j] = f_anggota_populasi_terurut[i][j];
            }
        }
        ...
        for (int i = 0; i < f_anggota_populasi_terurut_cr.length; i++) {
            System.out.print("populasi ke-" + (i + 1) + " : ");
            for (int j = 0; j < f_anggota_populasi_terurut_cr[i].length; j++) {
                System.out.print(f_anggota_populasi_terurut_cr[i][j] + " ");;
            }
            System.out.println();
        }

//++++++++++++++++++++++++++++++++++++++++ Proses Crossovernya++++++++++++++++++++++++++++++++++++++++++++++++++++        
        if (jum_cross > 1) { //jumlah crossover
        ...
        System.out.println("Setelah Crossover :");
        for (int i = 0; i < f_anggota_populasi_terurut_cr.length; i++) {
            System.out.print("populasi ke-" + (i + 1) + " : ");
            for (int j = 0; j < f_anggota_populasi_terurut_cr[i].length; j++) {
                System.out.print(f_anggota_populasi_terurut_cr[i][j] + " ");;
    ...
    public void do_mutasi(double mr) {
        ...
        f_anggota_populasi_mutasi = new double[f_anggota_populasi_terurut_cr.length][f_anggota_populasi_terurut_cr[0].length];

        for (int i = 0; i < f_anggota_populasi_mutasi.length; i++) {
            for (int j = 0; j < f_anggota_populasi_mutasi[0].length; j++) {
                f_anggota_populasi_mutasi[i][j] = f_anggota_populasi_terurut_cr[i][j];
            }
        }
        System.out.println();
    ...
    public double[][] bubblesort_fitness(double arr[], double[][] f_keanggotaan) {
        ...
        double[] temp_index;
        double temp_idx, temp_fss;
        int flag = 0, idx;
        //index + nilai fitness
        fitness_index = new double[jum_populasi][2];
        final_fitness_index = new double[jum_populasi][2];

        f_keanggotaan_terurut_populasi = new double[f_keanggotaan.length][f_keanggotaan[0].length];

        for (i = 0; i < jum_populasi; i++) {
            fitness_index[i][0] = i;
            fitness_index[i][1] = arr[i];
        }

        for (i = 0; i < n - 1; i++) {
            for (j = 0; j < n - i - 1; j++) {
                if (fitness_index[j][1] < fitness_index[j + 1][1]) {
                    temp_idx = fitness_index[j][0];
                    temp_fss = fitness_index[j][1];
//                    double temp = arr[j];
                    fitness_index[j][0] = fitness_index[j + 1][0];
                    fitness_index[j][1] = fitness_index[j + 1][1];
                    arr[j] = arr[j + 1];

                    fitness_index[j + 1][0] = temp_idx;
                    fitness_index[j + 1][1] = temp_fss;
                }
            }
        }
        ...
        for (int k = 0; k < f_keanggotaan_terurut_populasi.length; k++) {
            for (int l = 0; l < f_keanggotaan_terurut_populasi[k].length; l++) {
                idx = (int) fitness_index[flag][0];
                f_keanggotaan_terurut_populasi[k][l] = f_keanggotaan[idx][l];
            }
            flag++;
        }
